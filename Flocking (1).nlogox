...
<model version="NetLogo 7.0.3" snapToGrid="false">
  <code><![CDATA[; Flocking model with automatic swarm metrics
; Based on original NetLogo Flocking model by Uri Wilensky
; Modified for Experiment 2: Vision vs. Swarm Structure

globals [
  order              ; global alignment: 0 = random, 1 = perfect
  num-swarms         ; number of disconnected swarms
  max-swarm-size     ; size of the largest swarm
]

turtles-own [
  flockmates         ; agentset of nearby turtles
  nearest-neighbor   ; closest one of our flockmates
  group-id           ; component ID for swarm detection
]
...
to calculate-order
  if count turtles = 0 [ set order 0 stop ]
  let avg-x mean [ dx ] of turtles
  let avg-y mean [ dy ] of turtles
  set order sqrt (avg-x * avg-x + avg-y * avg-y)
end

to find-swarms
  ; Reset all group IDs
  ask turtles [ set group-id nobody ]
  set num-swarms 0
  set max-swarm-size 0
  
  ; Process all unassigned turtles
  while [ any? turtles with [ group-id = nobody ] ] [
    ; Start a new swarm
    let starter one-of turtles with [ group-id = nobody ]
    set num-swarms num-swarms + 1
    
    ; BFS initialization
    let visited turtle-set starter
    let frontier turtle-set starter
    
    ; Breadth-first search through connected agents (within vision)
    while [ any? frontier ] [
      let current one-of frontier
      ask current [ set group-id num-swarms ]
      set frontier frontier with [ self != current ]
      
      ; Find new neighbors of 'current' that are not yet in 'visited'
      ask current [
        let newly-seen turtles in-radius vision with [
          group-id = nobody and not member? self visited
        ]
        set visited (turtle-set visited newly-seen)
        set frontier (turtle-set frontier newly-seen)
      ]
    ]
    
    ; Update max swarm size
    let current-size count visited
    if current-size > max-swarm-size [ set max-swarm-size current-size ]
  ]
end
...
; See Info tab for full copyright and license.]]></code>
  <widgets>
    <view x="250" wrappingAllowedX="true" y="10" frameRate="30.0" minPycor="-35" height="506" showTickCounter="true" patchSize="7.0" fontSize="10" wrappingAllowedY="true" width="505" tickCounterLabel="ticks" maxPycor="35" updateMode="1" maxPxcor="35" minPxcor="-35"></view>
    ...
    <slider x="9" step="0.5" y="135" max="10.0" display="vision" height="33" min="0.0" direction="Horizontal" default="2.0" variable="vision" units="patches" width="223" sizeVersion="0"></slider>
    ...
    <monitor x="7" precision="17" y="323" height="60" fontSize="11" width="100" display="order">order</monitor>
    <monitor x="115" precision="17" y="323" height="60" fontSize="11" width="100" display="max-swarm-size">max-swarm-size</monitor>
    <monitor x="8" precision="17" y="390" height="60" fontSize="11" width="100" display="num-swarms">num-swarms</monitor>
  </widgets>
